...
export class ProjectAgent {
  ...
  async summarizeNote(itemId: string) {
    ...
    await this.prisma.item.update({ where: { id: itemId }, data: { raw: { ...(item as any).raw, summaryMarkdown: result.summaryMarkdown, tags: result.tags, noteType: result.noteType } } });
    return result;
  }
  ...
  async computeRisk(projectId: string) {
    ...
    const now = Date.now();
    const recentNotes = items.filter((i) => i.raw?.kind === 'NOTE' && (now - new Date(i.createdAt).getTime()) / (1000 * 60 * 60 * 24) <= 14);
    const riskTagCount = recentNotes.reduce((acc, i) => acc + (Array.isArray(i.raw?.tags) ? (i.raw.tags as string[]).filter((t) => /risk|block|issue/.test(t)).length : 0), 0);
    ...
    const userId = (await this.prisma.project.findUnique({ where: { id: projectId }, select: { userId: true } }))!.userId;
  ...
  async qa(projectId: string, query: string) {
    ...
    const sys = 'You answer user questions about a single project using the given context only.';
